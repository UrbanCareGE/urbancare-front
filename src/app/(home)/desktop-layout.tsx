...
const HeaderLogo = () => (
    <Link href="/" className="flex items-center">
        <span className="font-bold text-xl text-primary tracking-tight">URBANCARE</span>
    </Link>
);
        ...
        <nav className="flex items-center gap-1">
            {headerNavItems.map((item) => {
                ...
                const isActive = item.href === ""
                    ? pathname === `/apartment/${apartmentId}` || pathname === `/apartment/${apartmentId}/`
        ...
        <DropdownMenu>
            ...
            <DropdownMenuContent className="w-72" align="end" sideOffset={8}>
                ...
                <DropdownMenuItem asChild>
                    <Link href={`/apartment/${user?.selectedApartment?.id}/profile`} className="cursor-pointer">
