...
export type OptimisticUrgentItem = UrgentItemDTO & { _isPending?: boolean; _tempId?: string };
...
export function useCreateUrgent(userContext: UserContextType, onSuccess?: (urgent: UrgentItemDTO) => void) {
    ...
    const {mutate, isPending, isError, error} = useMutation({
        mutationFn: async ({content}: { apartmentId: string, content: string }) => {
            return await UrgentService.add(user?.selectedApartment?.id ?? '', content);
        },
        ...
        onMutate: async ({apartmentId, content}) => {
            ...
            const previousItems = queryClient.getQueryData<OptimisticUrgentItem[]>(queryKey);
        ...
        onSuccess: (urgentItemDTO, variables, context) => {
            ...
            queryClient.setQueryData<OptimisticUrgentItem[]>(
                context.queryKey,
                (prev) => prev?.map(item =>
                    item._tempId === context.tempId ? {...urgentItemDTO, _isPending: false} : item
                ) ?? [urgentItemDTO]
            );
