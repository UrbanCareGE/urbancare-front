...
const NavigationLinkInner = forwardRef<
  HTMLAnchorElement,
  NavigationLinkInnerProps
>(({ navigationItem, className, isActive, ...props }, ref) => (
  <Link
    ref={ref}
    {...props}
    className={cn(
      'group relative flex items-center gap-2.5 urbancare-rounded-panel px-2 py-1 transition-all duration-150 text-foreground-primary',
      isActive ? 'bg-primary-container/50' : 'lg:hover:bg-surface-variant',
      className
    )}
...
>(({ navigationItem, className, inSheet = true, ...props }, ref) => {
    ...
    return (
      <SheetClose asChild>
        <NavigationLinkInner
          ...
          className={cn('py-1.5 urbancare-text-3xl', className)}
          isActive={isActive}
          {...props}
        />
      </SheetClose>
  ...
  return (
    <NavigationLinkInner
      ref={ref}
      navigationItem={navigationItem}
      className={cn('urbancare-text-xl', className)}
      isActive={isActive}
      {...props}
    />
  );
