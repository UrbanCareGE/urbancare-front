...
const MobileSideBarRoot = ({
  children,
  side = 'right',
  defaultOpen = false,
}: MobileSideBarRootProps) => {
...
const MobileSideBarTriggerComponent = ({
  children,
  className,
...
const MobileSideBarContentComponent = ({
  children,
  className,
  ...
  return (
    <SheetContent side={side} className={cn('w-10/12 bg-text-bg')}>
      <VisuallyHidden>
...
const MobileSideBarHeaderComponent = ({
  children,
  className,
...
const MobileSideBarBodyComponent = ({
  children,
  className,
}: MobileSideBarBodyProps) => {
  return (
    <div
      className={cn('flex-1 w-full overflow-y-scroll bg-background', className)}
    >
      {children}
    </div>
  );
...
const MobileSideBarFooterComponent = ({
  children,
  className,
  ...
  return (
    <div
      className={cn(
        'h-auto w-full px-3 py-1 border-t bg-background border-border rounded-b-md',
        className
      )}
    >
      {children}
