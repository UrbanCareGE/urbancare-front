  ...
  return (
    <div className={cn('hidden lg:flex flex-wrap gap-2', className)}>
      {ALL_TAGS.map((tag) => {
        ...
        return (
          <button
            ...
            onClick={() => onClick(tag)}
            className={cn(
              'py-1 px-3 rounded-full text-sm font-medium transition-all lg:hover:scale-[1.03] lg:active:scale-95',
              isSelected
      ...
      {selectedTags.length > 0 && onClear && (
        <button
          onClick={onClear}
          className="py-1 px-3 rounded-full text-sm font-medium text-foreground-tertiary lg:hover:text-error lg:hover:bg-error/10 transition-all lg:active:scale-95"
        >
          გასუფთავება
        </button>
      )}
