...
export const AuthService = {
  ...
  nextLogin: async (loginReq: LoginDTO): Promise<ApiResponse<UserDTO>> => {
    return await api.post<UserDTO, LoginDTO>('/api/auth/login', loginReq, {
      server: true,
    });
  },
  ...
  generateOtp: async (phone: string): Promise<string> => {
    const { data } = await api.post<string>('/api/otp/generate', undefined, {
      params: { phone },
    });
    return data;
  },
  ...
  getChatInfo: async (apartmentId: string): Promise<ChatDTO[]> => {
    const { data } = await api.get<ChatDTO[]>(
      `/api/secure/chat/${apartmentId}/list`
    );
    return data;
  },
